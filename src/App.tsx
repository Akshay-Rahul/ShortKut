...
1. **Summary: 
   ...
   - Format the timestamps like "[00:01]" and begin each point with "At [timestamp], the video discusses...".
...
Here is the transcript of the video: ${transcripted}`;
    ...
    try {
      ...
      setErorr("There was an error generating the summary.");
      setLoad(false);
    }
  }
  ...
  const getTranscript = async (id: string) => {
    ...
    try {
      ...
      setErorr("Failed to fetch the transcript.");
      setLoad(false);
    }
  };
  ...
  const handilSubmit = async (e: any) => {
    ...
    if (videoId == null) {
      setErorr("You entered an invalid YouTube URL.");
      setLoad(false);
    } else {
      setErorr("");
